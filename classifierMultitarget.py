...
from generators import imageCubeGen, Batcher
...
def buildAutoencoder(inputShape):
	...
	d = Dense(16)(flat)
	candidate = Dense(1, activation='sigmoid', name='candidate')(d)



	autoencoder = Model(inputs=[input_img], outputs=[decoded, candidate], name='multiOut')
...
if __name__ == '__main__':
	...
	'''
	tGen = imageCubeGen(images, trainImagesDF, noduleDF, candidatesDF, cubeSize=cubeSize, autoencoder=True)
	vGen = imageCubeGen(images, valImagesDF, noduleDF, candidatesDF, cubeSize=cubeSize, autoencoder=True)
	dqt = Batcher(tGen, vGen, batchSize=args.batchSize)
	'''




	tCandidates = candidatesDF.merge(trainImagesDF, on='imgNum').copy(deep=True)
	vCandidates = candidatesDF.merge(valImagesDF, on='imgNum').copy(deep=True)

	tGen = candidateGen(imagesSSD, tCandidates, cubeSize=cubeSize, autoencoder=True)
	vGen = candidateGen(imagesSSD, vCandidates, cubeSize=cubeSize, autoencoder=True)
	print 'length of positive selection: ', len(tCandidates[tCandidates['class']==1])
	print 'length of positive selection: ', len(vCandidates[vCandidates['class']==1])

	#tGenCancerous = candidateGen(images, tCandidates[tCandidates['class']==1], cubeSize=cubeSize, autoencoder=True)
	#vGenCancerous = candidateGen(images, vCandidates[vCandidates['class']==1], cubeSize=cubeSize, autoencoder=True)


	#stratifiedQ = Batcher(tGen, vGen, trainStratified=tGenCancerous, valStratified=vGenCancerous, batchSize=args.batchSize)
	stratifiedQ = Batcher(tGen, vGen, batchSize=args.batchSize)
